...
parser MyParser(packet_in packet,
    ...
    state parse_ipv4 {
        ...
        transition select(hdr.ipv4.protocol) {
            ...
            TYPE_SNAPSHOT: parse_mySnapshot;
            TYPE_SIGNAL: parse_mySignal;
            default: accept;
        }
    }
    ...
    state parse_mySnapshot {
        packet.extract(hdr.mySnapshot);
        transition accept;
    }

    state parse_mySignal {
        packet.extract(hdr.mySignal);
        transition accept;
    }
...
control MyIngress(inout headers hdr,
    ...
    table check_snapshot_aggregate_satisfied {
        key = {
            hdr.mySnapshot.queryID: exact;
        }
        actions = {
            NoAction;
            doCheck;
    ...
    action doBcastSignal(bit<16> mgrp) {
        standard_metadata.mcast_grp = mgrp;
        hdr.mySignal.synack = 0x10;
    }

    table bcast_signal {
        key = {
            hdr.ipv4.dstAddr: exact;
            hdr.mySignal.synack: exact;
        }
        actions = {
            NoAction;
            doBcastSignal;
    ...
    apply {
        timestamp_t temp_time = 0;
        bit<16> temp_query_id = 0;
        bit<22> temp_snapshot = 0;
        ...
        if (temp_loss_count > LOSS_PER_COUNT) {
            ...
            if (hdr.ipv4.isValid()) {
                ...
                } else if (hdr.mySnapshot.isValid()) {
...
control MyEgress(inout headers hdr,
    ...
    action doSigUnpack(ip4Addr_t hostAddr) {
