...
def mpoll(destMAC, destIP, qid, timestamp, repollNumber):
    ...
    global polled_timestamp, snapshot_timestamp, consistency_diff
            ...
            if IP in reply:
                if reply[IP].proto == CTRL_PROTO:
                    ...
                    if fetched_timestamp[0] == Timestamp:
                        ...
                        polled_timestamp = struct.unpack('>L', bytes(reply[IP].payload)[7:10])[0]
                        if overflow_flags == 1:
                            print("Overflowed!")
                    ...
                    if fetched_seq[0] == Timestamp:
                        isPoll = True
                        snapshot_timestamp = struct.unpack('>L', bytes(reply[IP].payload)[2:5])[0]
                        if polled_timestamp > 0:
                            consistency_diff.append(abs(snapshot_timestamp - polled_timestamp))
                else:
...
if __name__ == "__main__":
    ...
    print("Consistency Diff = ", sum(consistency_diff) / len(consistency_diff))
