...
def writeMonResponse(p4info_helper, switch, dst_ip_addr, query_id, timestamp_num):
    """
    ...
    if timestamp_num == 1:
        table_entry2 = p4info_helper.buildTableEntry(
            table_name="MyIngress.console_reponse_timestamp",
            match_fields={
                "hdr.ipv4.dstAddr": dst_ip_addr,
            },
            action_name="MyIngress.record_first_timestamp",
            action_params= {
            })
        switch.WriteTableEntry(table_entry2)
    elif timestamp_num == 2:
        table_entry2 = p4info_helper.buildTableEntry(
            table_name="MyIngress.console_reponse_timestamp",
            match_fields={
                "hdr.ipv4.dstAddr": dst_ip_addr,
            },
            action_name="MyIngress.record_second_timestamp",
            action_params= {
            })
        switch.WriteTableEntry(table_entry2)
...
def main(p4info_file_path, bmv2_file_path):
    ...
    try:
        ...
        writeMonResponse(p4info_helper, switch=s2, dst_ip_addr="10.1.2.2", query_id=1, timestamp_num=1)
        writeMonResponse(p4info_helper, switch=s3, dst_ip_addr="10.1.3.3", query_id=1, timestamp_num=2)
