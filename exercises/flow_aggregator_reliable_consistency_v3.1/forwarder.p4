...
control MyIngress(inout headers hdr,
    ...
    table console_reponse_timestamp {
        ...
        actions = {
            NoAction;
            record_first_timestamp;
            record_second_timestamp;
        }
        size = 32;
        default_action = NoAction();
    }
                ...
                if (hdr.myControl.isValid()) {
                    ...
                    if (aggr_query_id > 0) {
                        ...
                        if (total_monNum > 0 && seen_monNum + 1 >= total_monNum) {
                            ...
                            queryCounters.write((bit<32>)aggr_query_id, 0);
                            acked_monitor_number.write((bit<32>)aggr_query_id, 0);
                            if (hdr.myControl.timestampA == 0) {
                                temp_timestamp.read(hdr.myControl.timestampA, 0);
                            } else if (hdr.myControl.timestampB == 0) {
                                temp_timestamp.read(hdr.myControl.timestampB, 0);
                            }
                        } else {
                            if (hdr.myControl.timestampA != 0) {
                                temp_timestamp.write(0, hdr.myControl.timestampA);
                            } else if (hdr.myControl.timestampB != 0) {
                                temp_timestamp.write(0, hdr.myControl.timestampB);
                            }
                            control_drop = 1;
                        }
                    }
